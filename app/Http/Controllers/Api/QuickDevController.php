...
/**
 * 快速接口
 * Class QuickDevController
 *
 * @package App\Http\Controllers
 */
class QuickDevController extends Controller
{
    ...
    public function login(Request $request)
    {
        if ($request->method() == 'POST')
        {
            ...
            if ($exists) {
                ...
                User::query()->where('id', $user->id)->update(['ssruuid' => $ssruuid, 'remember_token' => $remember_token, 'last_login' => time()]);
                ...
                if (!empty($userLabelIds)) {
                    ...
                    foreach ($nodeList as &$node) {
                        $ssrList[] = [
                            ...
                            'compatible'   => $node->compatible ? 1 : 0,
                            'status' => $node->status
                        ];
                    }
                }
