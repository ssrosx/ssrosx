...
/**
 * 快速接口
 * Class QuickiOSDevController
 *
 * @package App\Http\Controllers
 */
class QuickiOSDevController extends Controller
{
    ...
    public function login(Request $request)
    {
        if ($request->method() == 'POST')
        {
            ...
            if ($exists) {
                ...
                $remember_token_ios = makeRandStr(20);
                User::query()->where('id', $user->id)->update(['ssruuid' => $ssruuid, 'remember_token_ios' => $remember_token_ios, 'last_login' => time()]);
    ...
    public function quick(Request $request)
    {
        if ($request->method() == 'POST')
        {
            ...
            if ($exists) {
                ...
                // 更新登录信息
                $remember_token_ios = makeRandStr(20);
                User::query()->where('id', $user->id)->update(['remember_token_ios' => $remember_token_ios, 'last_login' => time()]);
...
//                $account_status = 0;
            ...
            $user->remember_token_ios = makeRandStr(20);
            $user->save();
...
//            usleep(5000000);
    ...
    private function http_post_data($data_string, $istest) {
        ...
        Log::info('iOS Dev AppStore $response_json: ' . $response_json);
    ...
    {
        if ($request->method() == 'POST') {
            if (self::$systemConfig['is_china_local'])
            {
                return Response::json(['status' => self::$success_code, 'data' => '', 'message' => self::$none_code]);
            }
            else
            {
                        ...
                        if($product_id && !empty($product_id)){
                            ...
                            if ($goods)
                            {
                                ...
                                try {
                                    ...
                                    $user = User::query()->where('remember_token_ios', $token)->where('status', '>=', 0)->first();
                                    if (!$user) {
                                        return Response::json(['status' => self::$faulure_code, 'data' => '', 'message' => self::$id_user_not_exist]);
                                    }
